...
Two full-season `runs ⋈ dungeon_data` scans (`DUNGEON_UPGRADES[_PER_KEYLEVEL]_SQL`, ~2-3 min each at 10-20M runs) used to run ~12x per build across the dashboard/index/dungeon/item steps (worst case `createDungeonOverviewImg` re-ran the per-level scan once per dungeon). Now `fetch_runs_per_dungeon`, `fetch_runs_per_dungeon_per_level[_above_level]`, and `fetch_total_season_runs` read the ~240-row nightly rollup `aggregated_runs_per_dungeon_per_level` (built by `sp_agg_runs_per_dungeon_per_level`, registered in `sp_run_agg_pipeline`, current season only, shadow+RENAME like [[aggregation-pipeline]]).

Why: the DB runs at 100% load and cannot be scaled, so every redundant query removed is a direct win. The user rejected query parallelism for the same reason (keystone.guru thumbnail HTTP is threaded instead, zero DB load). Thumbnails cannot be cached across builds (they differ every time).
...
Note: the original one-off migration `2026-07-19_runs_per_dungeon_per_level.sql` has been folded into `backend_scripts/database.sql` (the `migrations/` dir is now empty), so the proc and table live in the main schema file. Adoption-rate denominators that consume these globals are in [[item-page-aggregate-perf]].
