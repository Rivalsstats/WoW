...
1. **`wipe_watch` owns the only path that un-pauses `WRITE_GATE`.** If it dies, collection halts silently forever, because `asyncio.gather(..., return_exceptions=True)` in `main()` swallows the traceback and the failsafe lives in the same loop. Every tick is individually try/except'd and a `finally` resumes the gate on any exit. Never add an un-guarded `await` to that loop (`collectLeaderboardData.py`, `wipe_watch()` around line 2244).
