...
- NON-EXISTENT node filtering: `write_talent_trees` passes the baked `nodes` dict through `commonUtils.filter_talent_tree_nodes` so the analyzer (and the tierlist modal, [[tierlist-gear-modal]]) hide the same stray-questionmark / padding-entry nodes the spec page tree does (`node_has_valid_spellid` + the entry-identity strip, both shared from `commonUtils` and reused by `generateSpecPages.build_ui_tree` / `strip_empty_talent_entries`). CRITICAL: `fullNodeOrder` is left COMPLETE (it is the loadout-decode bit-alignment source; dropping a node from it garbles every build). A node in `fullNodeOrder` but absent from `nodes` is simply not drawn — `ttNode()` returns null and the render (`ttColumn`/`heroColumn`/`ttTaken`/`ttHeroTree`) skips it, in both `analyzer.js` and `tierlist-modal.js`, so the filter lives only at the bake.
