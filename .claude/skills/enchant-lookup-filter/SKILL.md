...
Dropping enchant IDs that are not in `data/static/enchantments.json` (spec page build, `fetch_enchant_info` in `backend_scripts/generateSpecPages.py`) is the *intended* mechanism for hiding old-expansion enchants people still have equipped on normally-unenchantable slots (HANDS/WAIST/WRIST noise). The user rejected building supplement files or wago.tools imports for IDs absent from the raidbots file: "raidbots is not missing any data." A warning log for each skipped ID (in `fetch_enchant_info`) is all that is wanted. Do NOT propose auto-generating enchant metadata from other sources.
