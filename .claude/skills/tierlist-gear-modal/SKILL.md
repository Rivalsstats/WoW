...
- `top50` — the top-50 verified players' actual loadout (`_top50_gear` + `_top50_talents`): the per-slot most-common equipped item (with its bonus ids) from `top_player_loadout_items`, worn with the most-common `top_player_loadouts.loadout_key` (a real, stored Blizzard v2 code, not a synthesized one), read via `databaseConnector.fetch_top50_loadouts`. Enchants and gems are NOT stored per slot in the top-50 tables (gems are a per-player `{gem_item_id, usage_count}` bag), so `top50` fills them the same way `popular`/`simcbis` do: the top-50 `enchant_map` + `gem_ranking` from `_prepare_spec`, applied over the equipped set by socket budget (`apply_enchants_and_gems`).
